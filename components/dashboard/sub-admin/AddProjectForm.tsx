...
export default function AddProjectForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await showAlert({ title: 'Project created', message: 'Project has been added to accounts successfully.', variant: 'success' });
      router.push('/subadmin/accounts?tab=projects');
    } catch (error: any) {
      showAlert({ title: 'Create failed', message: error.message || 'Failed to create project.', variant: 'danger' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      <div className="h-full min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar">
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-4 py-4 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-2 border-b border-slate-100 bg-slate-50/60 px-6 py-5">
              <h2 className="text-xl font-bold tracking-tight text-slate-900">Add New Project</h2>
              <p className="text-xs font-medium text-slate-500">Create a construction or event project for fundraising and accounts tracking.</p>
            </div>
                ...
                <div className="md:col-span-2">
                  <Field label="Project photos">
                      ...
                      {previews.length > 0 ? (
                        <div className="grid w-full grid-cols-2 gap-3 p-3 md:grid-cols-5">
                          {previews.map((preview, index) => (
                            <div key={`project-photo-${index}`} className="group relative aspect-video overflow-hidden rounded-md border border-slate-200 bg-white">
                              <img src={preview} alt={`Project photo ${index + 1}`} className="h-full w-full object-cover" />
                              <button
                                type="button"
                                onClick={(event) => {
                    ...
                    {selectedFiles.length > 0 && (
                      <p className="ml-1 text-[11px] font-semibold text-slate-400">{selectedFiles.length}/5 photos selected</p>
                    )}
                  </Field>
                </div>
